''''
    In this code I'll plot the evolution of the Weighted f1 score for the MultiClass Classification
    Depending on the Size Embedding Hyperpameter that defines the dimensions of linear transformations in the SAGE Layers
'''
...
for size_embedding in range(177, 180):
    ...
    for nb_files in range(file_count):
        ...
        print("weighted f1-score : ", sklearn.metrics.f1_score(edge_label1, pred1, labels = list(range(15)), average = 'weighted'))
        ...
        # -------------------------------------------- ---------------------------------------- -----------------------------------------------------
        weighted_f1_mean += sklearn.metrics.f1_score(actual1, test_pred1, labels = list(range(15)), average = 'weighted')
...
plt.title(f'''
E-GraphSAGE
Best Weighted F1_Score = {weighted_f1_axis[np.argmax(weighted_f1_axis)]} and is given with Size Embedding = {size_embedding_axis[np.argmax(weighted_f1_axis)]}''')
plt.xlabel("Size Embedding")
