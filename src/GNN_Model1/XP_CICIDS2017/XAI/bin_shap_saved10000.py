...
class GPreprocessing():
    ...
    def train(self, data1):
        ...
        self.cols_to_norm1 = list(set(list(data1.iloc[:, :].columns )) - set(list(['label', ' Source IP', ' Destination IP'])) )
        data1[self.cols_to_norm1] = self.scaler1.fit_transform(data1[self.cols_to_norm1])
        ## Create the h attribute that will contain the content of our flows
        data1['h'] = data1[ self.cols_to_norm1 ].values.tolist()
        # We need to delete all the attributes (cols_to_norm1) to have the {Source IP, Destination IP, label, h} representation
        data1.drop(columns = self.cols_to_norm1, inplace = True)
    ...
    def test(self, data1):
        ...
        data1[self.cols_to_norm1] = self.scaler1.transform(data1[self.cols_to_norm1])
        data1['h'] = data1[ self.cols_to_norm1 ].values.tolist()
        data1.drop(columns = self.cols_to_norm1, inplace = True)
        columns_titles = [' Source IP', ' Destination IP', 'h', 'label']
        data1 = data1.reindex(columns=columns_titles)
...
class Model(nn.Module):
    ...
    def xai_predict(self, data1):
        ...
        data1[' Destination IP'] = dst_ip
        data1['label'] = data_labels
        G1_test = self.preprocessing.test(data1)
        G1_test = G1_test.to('cuda:0')
        node_features_test1 = G1_test.ndata['feature']
        edge_features_test1 = G1_test.edata['h']
...
#X1_train_batched.to_csv(f'./src/GNN_Model1/XP_CICIDS2017/XAI/SHAP_SAVED/Train_all_final.csv', sep=',', index = False)
X1_test.to_csv(f'./src/GNN_Model1/XP_CICIDS2017/XAI/SHAP_SAVED/Test_all_final.csv', sep=',', index = False)
...
import pickle
filename_expl = './src/GNN_Model1/XP_CICIDS2017/XAI/SHAP_SAVED/GNN_SHAP_explainer_all_final.sav'
pickle.dump(explainer, open(filename_expl, 'wb'))
filename = './src/GNN_Model1/XP_CICIDS2017/XAI/SHAP_SAVED/GNN_SHAP_shapvalues_all_final.sav'
